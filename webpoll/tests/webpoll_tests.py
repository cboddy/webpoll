...
class WebPollTest(unittest.TestCase):
    ...
    def test_filter_links(self):
        ...
        self.assertIn("https://icomp.de/shop-icomp/en/33/items/commodore-back-in-germany.html", links)

    def test_msg_builder(self):
        notifications = [webpoll.Notification(url, link) for (url,link) in [("a", "b"), ("c", "d")] ]
